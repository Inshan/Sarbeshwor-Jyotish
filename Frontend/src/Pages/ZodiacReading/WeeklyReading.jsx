        ...
        try {
            ...
            if (response.data) {
                const DocumentData = response.data.filter(
                    (item) => item.status === "Publish" && item.page_type === "WeeklyRashifal"
                );
                setData(DocumentData[0]); // Assuming you want to slice the filtered date
            }
            if (response.data) {
                const DocumentData1 = response.data.filter(
                    (item) => item.status === "Publish" && item.page_type === "WeeklyRashifalZodiac"
                );
...
{/* 
            ...
            {/* </div> */}
            <div className='container pt-16 pb-8 bg-asheshColor'>
                {data1.map((item) => (
                    <div className='flex border-b pb-2 mb-5'>
                        <div className='pe-10'>
